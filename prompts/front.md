...
- 只有当前或紧邻上下文明确要求检查、修改、合并、推送、发布、部署或变更权限，或明确批准上一条 Agent 的具体方案时，才进入 worker；模糊闲聊不得用 `need_more_context=true` 兜底。
...
- 已明确提出真实任务但执行所需材料不足、证据矛盾、无法在只读本地代码中可靠回答，或疑似越权操作。恶意攻击、凭据外传和破坏性请求按上面的直接拒绝规则处理。若连真实任务意图都不明确，不得因“上下文不足”升级 worker，应 `route=reply` 先自然澄清。

...
- “看看、改改、搞下、弄一下、拿来试试、继续”等泛口语单独出现不构成修改授权。没有明确对象、范围、结果或交付要求时，必须 `route=reply` 或 `no_reply`、`execution=read_only`，不得升级 worker、访问仓库或调用外部系统。
...
- 当前消息若是“已经提测了”“你试试”“还需要吗”等状态确认或前一动作回执，优先按确认/只读回复，不被更早的执行请求带入 worker。

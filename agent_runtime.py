...
from agent_core import sanitize_text
...
def _command(profile: AgentProfile, schema: Mapping[str, Any]) -> list[str]:
    ...
    if protocol == "claude-stream-json":
        ...
        if profile.read_only:
            command.extend(["--permission-mode", "plan", "--tools", "Read,Grep,Glob"])
        elif profile.options.get("permission_mode"):
            command.extend(["--permission-mode", str(profile.options["permission_mode"])])
        if profile.options.get("dangerously_skip_permissions"):
            command.append("--dangerously-skip-permissions")
        command.extend(profile.extra_args)
        return command
    if protocol == "custom-jsonl-v1":
        return [*launcher, *profile.extra_args]
    if protocol != "pi-rpc":
        raise ValueError(f"unsupported agent protocol: {protocol}")
    command = [*launcher, "--mode", "rpc", "--no-session"]
    if profile.model:
        command.extend(["--model", profile.model])
    ...
    if profile.read_only:
        ...
        command.extend(["--tools", str(profile.options["tools"])])
    if profile.options.get("provider"):
        command.extend(["--provider", str(profile.options["provider"])])
    command.extend(profile.extra_args)
    return command
